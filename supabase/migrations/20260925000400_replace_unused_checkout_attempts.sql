-- An attempt with no Stripe session was never shown to the owner, so it can be
-- replaced once it is too close to expiry for Stripe's 30-minute minimum
-- expires_at (or already expired, when no checkout.session.expired webhook will
-- ever arrive to clear it). A fresh attempt is still reused, so a double submit
-- keeps the same attempt_id and therefore the same Stripe idempotency key.
...
begin
  ...
  if found and current_attempt.stripe_checkout_session_id is null
    and current_attempt.checkout_expires_at < now() + interval '30 minutes 30 seconds' then
    delete from public.stripe_checkout_attempts a where a.attempt_id = current_attempt.attempt_id;
    current_attempt := null;
  end if;

  if current_attempt.attempt_id is null then
