-- F035: Coastal, Riviera, Velvet and Black Tie guest themes. Existing weddings keep their saved theme.
alter table public.weddings drop constraint wedding_theme_valid;
alter table public.weddings add constraint wedding_theme_valid check (theme in (
  'minimal', 'romantic', 'bold', 'alcantara', 'countryside', 'evening-gold', 'terracotta', 'heather',
  'coastal', 'riviera', 'velvet', 'black-tie'
));
...
begin
  ...
  loop
    if theme_name not in ('minimal', 'romantic', 'bold', 'alcantara', 'countryside', 'evening-gold', 'terracotta', 'heather',
  'coastal', 'riviera', 'velvet', 'black-tie')
      or jsonb_typeof(theme_value) is distinct from 'object' then return false; end if;
