...
create function public.expire_checkout_attempt(requested_attempt_id uuid, requested_session_id text)
returns boolean
language plpgsql security definer set search_path = '' as $$
...
language sql stable security definer set search_path = '' as $$
  select
    exists (
      select 1 from public.stripe_payments p join public.weddings w on w.id = p.wedding_id
      where w.owner_id = (select auth.uid()) and p.revoked_at is null and p.expires_at > now()
    ),
    (
      select max(p.expires_at) from public.stripe_payments p join public.weddings w on w.id = p.wedding_id
      where w.owner_id = (select auth.uid()) and p.revoked_at is null
    ),
    (
      select p.revoked_reason from public.stripe_payments p join public.weddings w on w.id = p.wedding_id
      where w.owner_id = (select auth.uid()) order by p.paid_at desc, p.created_at desc limit 1
    );
$$;
