...
Landing and Details use the same compact Save the date / Details navigation whenever Details is enabled. The Details page opens with the couple names and a short editorial introduction, followed by only the sections the couple filled in. F011 adds the existing wedding photo as a desktop side panel/mobile banner, with a botanical fallback, plus original line icons for the information cards. Modern Minimal uses warm paper/sage, Romantic cream/rose and arched photography, and Bold deep teal with pale and citrus cards. Mobile uses one vertical column with comfortable link targets; desktop uses two columns while FAQs span the available width. Theme changes never alter content or available actions. Photos use the same authorised, uncached photo endpoints as the landing page.
