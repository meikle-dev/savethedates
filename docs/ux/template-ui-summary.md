...
F044 (25 September 2026): while replies are open the Save the Date shows an outlined **RSVP now** action under the date, location and message, in the announcement's own text colour for every theme, with "Please reply by <date> (23:59 UTC)" when a closing date is set. The RSVP introduction says "Please reply by <date>." with the exact cutoff beneath ("Replies close at 23:59 UTC on <date> (<UK and Irish time>)"); without a date no deadline is mentioned. The name field adds "Replying for more than one person? Send one reply each." Rejected submissions keep the guest's own entry. Success replaces every pre-submit instruction ("Your reply has been sent."), moves focus to **Thank you**, names the saved answer, and offers **Reply for someone else** (a fresh, empty form that saves only when sent) and **View the wedding details** only while Details are visible. Closed states distinguish "RSVPs have closed" (with the passed date) from "RSVPs aren't open" (RSVP off). The owner preview mirrors the action and deadline but never submits; marketing examples show no RSVP action.
