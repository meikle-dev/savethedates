...
**SaveTheDates is live and public at `https://savethedates.co.uk` (opened 26 September 2026).** The owner confirmed a live Stripe purchase and refund on production, then removed the lock. The site is indexable, and Google's brand verification is approved and published. The launch record is in F009; the setup is in F041. **F009 is Done** (signed off by the owner 26 September 2026); Search Console indexing, the link-preview check (F047), and the deferred gates (F048, F054) continue as their own tickets rather than blocking it. The launch followed the [launch plan](launch-plan.md). F001-F009, F011-F032, F034-F037, F040, F042-F046, F055-F068 except F047-F048, F050 and F054 (still open; see each ticket) are Done. The [24 September assessment and delivery order](#24-september-walkthrough-assessment) takes precedence over the historical placement of entries below. **Owner staging check (26 September 2026):** everything on the checklist passed except two items, both since resolved: the refund test refunded a different, older test payment (investigated as F067, now closed) and mail to `hello@savethedates.co.uk` wasn't received then; forwarding has since been set up and an outside test confirmed (F066, Done). The results are in F041. **Next engineering work: F065** (separate Save the Date, Invitation and RSVP links, raised by the owner on 26 September 2026). **F068** (meal choices and food preferences on the RSVP) is Done and live in production (26 September 2026). **F069** (RSVP replies without JavaScript) is Ready. **F070** ("What we offer" showcase page, owner request) is Planned: UX can start now, and the build follows F068 and F065. **F038 is In Progress:** the owner confirmed Sentry works on staging; the checks that need the Sentry connector remain. **F061** is Done: the owner completed a real Stripe test checkout on staging and confirmed the fix works; independent review of the migration was deferred by the owner into F054. **F047 is In Progress:** production is open, so its messaging-app preview check can run now (launch plan stage 6, step 4). **F055 (Google sign-in) is Done:** owner confirmed a real sign-in on production. **F063 is Done:** Google's brand verification is approved and published, and the owner confirmed the production sign-in shows "SaveTheDates". **F066 is Done:** forwarding is set up and an outside test email was confirmed. The owner approved the legal text (25 September 2026) and re-approved the terms line that lists the Invitation (26 September 2026). The owner deferred F048 (customer data) and F054 (security review), and opened the site without them on 26 September 2026; F049 (real devices/assistive technology) was removed from the backlog by owner decision on 26 September 2026. F033 is Ready but follows launch work, and F039 is optional. F048/F050 are assessed tickets, not implemented fixes. F051-F053 are report-only growth tickets that don't gate launch; **F051 is In Progress** and waits for the owner's keyword-volume input. The domain is `savethedates.co.uk`. F054 (full security review) was a paid-launch gate; the owner deferred it, and it should run soon after launch.
...
- **Owner check (26 September 2026):** after the staging checklist, the owner reports Sentry logs are working fine on staging.
- **Next:** record the remaining specific checks (a thrown Server Action error, a browser error with a resolved source map, a rejected-webhook log and the three MCP queries). They need a session with the Sentry connector. Then mark F038 Done.
...
2. In each project, open **Authentication** and set:
   ...
   - **Sign in / Providers → Google** (F055, wanted at launch): set up after the Google client below.
   - **Email Templates:**
   ...
   - **Google sign-in** (F055, wanted at launch by the owner on 25 September 2026; production uses the custom domain from F063). Do this once per environment, staging first:
...
Updates 1, 2 and 4 are delivered by [F056](#f056---publish-the-tested-image-and-protect-staging). Update 3 is built, and the owner approved its text on 25 September 2026 (below).
...
- **Verified by curl after the above deployed:** staging `/` returns 401 with no login and 200 with the staging login; `/api/health` returns 200. The journeys were exercised afterwards by the owner (25 and 26 September 2026, below).
...
- **Owner staging check (26 September 2026), against the checklist Claude Code prepared:**
  - **Passed:**
    - email sign-up with confirmation, password reset, and email delivery to new accounts (in the inbox, not spam);
    - photo uploads (F062);
    - guest pages and a guest RSVP, and a switched-off Invitation giving "page not found" (F060);
    - test checkout, publish and the guest links;
    - Sentry logs (F038);
    - favicon and the 404 page (F047);
    - the marketing examples, including the Invitation examples.
  - **Not passed:**
    - **Refund test.** The owner refunded a payment in Stripe's test mode, and the site stayed published. Staging's database shows the webhook worked, but the refund was for a different, older test payment that staging never recorded as a purchase, so correctly nothing was revoked (investigation in F067). **Repeat it:** in Stripe's test mode, refund the £29.00 payment made on 25 September 2026 at 21:55 UTC (22:55 UK time), whose metadata has the test wedding's `wedding_id`. The site should unpublish within a minute, and the guest link should stop working.
    - **Support mailbox.** A test email to `hello@savethedates.co.uk` went nowhere, because nothing receives mail for the domain yet (F066).
  - **Owner decisions:**
...
- **Remaining acceptance check:** on a reachable staging URL, manually share a disposable published wedding link in WhatsApp and one app that fetches previews server-side (for example Slack); confirm names, date and static theme card only, and confirm an unavailable link gives generic metadata. Record the result here. Do not mark F047 Done until this is verified. **Blocker (26 September 2026):** staging and the locked production site both require a password, which messaging apps can't supply, so the preview can't be fetched. The owner confirmed the favicon and the 404 page on staging. Next: do the preview check right after production is unlocked (launch plan stage 6); eligible launch work may continue while it waits. F065 adds a second (Invitation) link, which gets the same check.
...
**Status:** Done (26 September 2026). Live on production: the Supabase Google provider is enabled, the consent screen is published ("In production"), and brand verification is approved and published (F063). Owner confirmed a real Google sign-in on production.
...
**Update (26 September 2026):** Owner completed "To finish" items 1–3 on staging: Google Cloud project `savethedates`, OAuth client "SaveTheDates staging", client ID/secret in Supabase's Google provider, `AUTH_GOOGLE_ENABLED=true` on the staging Render service. Full details in F041's owner setup progress. Production's own OAuth client, consent screen (published via brand verification, F063) and Google sign-in are live.
- **Signed off (26 September 2026):** owner confirmed a real Google sign-in on production. F055 is Done.

## F009 - Launch and operate the service

**Status:** Done (26 September 2026; signed off by the owner). SaveTheDates is live at `https://savethedates.co.uk`. Not completed and explicitly accepted by the owner as deferred rather than passed: F048 (customer data lifecycle), F054 (security review, which also carries F061's migration review), the independent release review, backup/restore drills, and source/commercial permission evidence for supplied botanical/backdrop assets. F049 (real-device/assistive-technology checks) was removed from the backlog by owner decision. Remaining open work is tracked on those tickets and on F038/F047.
...
- Every shipped supplied botanical/backdrop, including unused public assets, has source and commercial permission recorded or is removed/replaced with approved material; naming a supplier alone is not rights evidence.
...
- **Launch (26 September 2026).** SaveTheDates opened to the public at `https://savethedates.co.uk`.
  ...
  - **Resolved since launch:** the production Google sign-in check (F063, F055), an outside test email to `hello@` (F066), and the staging Stripe checkout fix (F061).
  - **Still open, tracked on their own tickets rather than blocking this one:**
    ...
    - The deferred gates: F048 (customer data lifecycle) and F054 (security review, which also carries F061's migration review).
    - Independent release review, and the backup and restore drills in the Done-when above.
  - **Signed off (26 September 2026):** the owner accepted the site as launched and working, with the items above explicitly deferred rather than resolved. F009 is Done on that basis.
...
**Status:** Done (25 September 2026; owner requested it as "the last missing piece of functionality"; the Product Manager made the decisions below)
...
- **Checks** (Node 24.11.0 and local Supabase; Chromium 1194):
  ...
  - Full browser suite against the production build (`E2E_PRODUCTION=1`): after the review fixes and rebuild, every test passed except the new focus-ring check, which read the ring after a mouse click, when Chromium doesn't apply `:focus-visible` to programmatic focus. The check now moves focus by keyboard, and `tests/invitation.spec.ts` plus `tests/dashboard.spec.ts` pass 14 of 14. Totals: 94 passed, and 20 development-only `/demo` tests skipped by design.
...
- **Owner decisions (26 September 2026):**
  - The terms line listing the Invitation in what £29 includes is re-approved.
  - The Save the Date should not link to the Invitation: "the two are separate". This also covers the navigation, which is F065.
- **Owner staging check (26 September 2026):** guest pages work; switching the Invitation off gave guests "page not found", as designed.
- **Staging:** migration `20260925000600` was applied through the Supabase connector before the code deploys. The recorded version is `20260925000600`, and the statements' MD5 matches the file. The old staging code ignores the new column, so applying it first is safe.
- **Next:** none for F060. Separate links for the Save the Date and the Invitation are F065.

## F061 - Staging checkout rejected by Stripe

**Status:** Done (26 September 2026). Both causes fixed on staging, and the owner completed a real Stripe test checkout on staging and confirmed it works. Independent review of migration `20260925000400` was deferred by the owner into the F054 security review rather than blocking this ticket.
...
**Done when:** On staging, a test checkout with card `4242 4242 4242 4242` opens Stripe Checkout, completes, and returns to Publish. The webhook grants the entitlement, and the site can be published. The logs show the request-ID trail from checkout created to entitlement granted, with no `payment.stripe.failed`. `npm run test:integration` passes, and an independent review of the migration finds no Blocking issues.
**Signed off (26 September 2026):** the owner completed a real Stripe test checkout on staging and confirmed it works; `npm run test:integration` passed 23 of 23. Independent review of migration `20260925000400` did not run separately — deferred by the owner into the F054 security review, which remains outstanding.

## F062 - Accept large photos by resizing them in the browser

**Status:** Done (26 September 2026; the owner confirmed uploads on staging)
...
- **Photo display confirmed (25 September 2026):** after migration `20260925000500`, the owner confirmed uploaded photos display on staging.
- **Owner check (26 September 2026):** the staging checklist asked for large photos from iPhone Safari, Android Chrome and a desktop browser other than Chrome, and the owner reported "photo upload works fine". The individual devices weren't listed separately.
- **Next:** none.

## F063 - Show SaveTheDates, not the Supabase address, in Google sign-in

**Status:** Done (26 September 2026). Brand verification approved and published; owner confirmed a real Google sign-in on production shows the consent screen naming "SaveTheDates".
...
- **Signed off (26 September 2026):** owner confirmed a real Google sign-in on production shows the consent screen naming "SaveTheDates".
...
- While the Invitation is on, the suggested share message links to `/invitation` (`src/features/workspace/share-message.ts`), so couples can no longer send a plain Save the Date.
- The Save the Date's navigation links to the Invitation, and anyone with the Save the Date link can reach `/invitation` by editing the URL.

**Decisions (Product Manager, 26 September 2026):**

1. **Two links, each with its own secret.**
   - The Save the Date link keeps the current secret (`/<names>/<secret>`).
   - A new, separate secret gives the Invitation link, which opens the Invitation first. Neither link can be worked out from the other.
   - Rejected: one secret with a different starting page. A guest could reach the Invitation by editing the URL, and a shared Details page couldn't tell which navigation to show.
2. **What each link opens:**

   | Page | Save the Date link | Invitation link |
   | --- | --- | --- |
   | Save the Date | Yes (first page) | No |
   | Invitation | Never | Yes (first page) |
   | Details, while on | Yes | Yes |
   | RSVP, while on | Only while the Invitation is off | Yes |
...
3. **Workspace.** The single guest-link panel becomes two, "Send your Save the Date" and "Send your Invitation", plus the RSVP link panel in decision 9.
   ...
   - Replies through either link go to the same guest list.
4. **Messages.**
   - The Save the Date message stays "Save the date! …" with its link, and mentions RSVP only while the Save the Date link offers it.
   - The Invitation message is "You're invited! …" with the Invitation link.
5. **Link previews (F047).** The Invitation link's preview names the Invitation rather than the Save the Date. The rules are unchanged: names and date only, and a static theme card.
6. **Owner previews.** The Save the Date preview's navigation matches the Save the Date link, and the Invitation preview matches the Invitation link.
7. **Wording.** Replace "one link" wording with two links sent at different times, wherever it appears:
...
- Each link opens only its pages in the table above.
  - This is checked for every combination of Invitation, Details and RSVP switched on or off, at mobile and desktop widths.
  - An Invitation URL under the Save the Date secret, and a Save the Date URL under the Invitation secret, give the generic "page not found".
- The server enforces the table. Invitation content is returned only for the Invitation secret, and RSVP accepts a reply only through a link that shows RSVP. Integration tests cover:
...
- The docs are updated (`docs/ux/template-ui-summary.md`, and `docs/overview/architecture.md` if the routes change).

**References:** F042 (share panel), F043 (guest URL), F044, F047, F060. Code:

- `src/features/weddings/guest-link.ts`
...
**Status:** Done (26 September 2026). Forwarding is set up and an outside test email arrived in the owner's Gmail, not in spam.
...
- Nothing receives mail for the domain yet.
- Resend is set up only to send (its records are on the `send` subdomain), and Resend receiving is off.
- The test email wasn't delivered anywhere. Gmail may show a bounce ("delivery failure") for it.
- DNS couldn't be checked from Claude Code's session.

**Decision (Product Manager, recommended): forward to the owner's Gmail, with no new mailbox subscription.**

1. **Set up free forwarding** from `hello@savethedates.co.uk` to `rmeikle55@gmail.com`.
   - Use GoDaddy's email forwarding if it's included free with the domain; otherwise use a free forwarding service such as ImprovMX.
   - Either one adds mail (MX) records to the root domain.
   - Don't change the Resend records: `send` MX and TXT, `resend._domainkey` and `_dmarc`.
   - If the forwarder asks for an SPF record on the root domain, add it to an existing one rather than creating a second.
2. **Optional: reply as hello@.** In Gmail → Settings → Accounts → **Send mail as**, add `hello@savethedates.co.uk` using Resend's SMTP: `smtp.resend.com`, port 465, username `resend`, and a new Resend API key named `gmail-support` with sending access only.
3. **A paid mailbox is not needed at launch.** Google Workspace or Microsoft 365 (about £5–6 a month) is only worth it if the owner wants a separate inbox.

**Done when:**

- A test email from an outside address to `hello@savethedates.co.uk` arrives in the owner's Gmail, not in spam.
- The option chosen and the records added are recorded in `release-inputs.md` section 3.
- A sign-up email still arrives, which shows sending wasn't broken.
- If "Send mail as" is set up, a reply sent as hello@ arrives without a spoofing warning.
...
- **Signed off (26 September 2026):** owner confirmed a test email from an outside address arrived. "Send mail as" (step 2) is optional and not set up.

## F068 - Meal choices and dietary requirements on the RSVP

**Status:** Done (26 September 2026). Live in production on image `fc45b5d`; the owner checked it on staging.
...
- **Owner check on staging (26 September 2026):** "the menu stuff we added all works really well."
- **Production (26 September 2026):**
  - The owner promoted production to image `fc45b5d` (digest `sha256:e19e1b8b…c4d9`, `dep-das1cbjncjis73fg1hm0`, live 19:00 UTC).
  - Migration `20260926000100` was applied to `msrpxvlxojnnefezestn` through the Supabase connector at 19:02:57 UTC. It's recorded as version `20260926190257`, following production's apply-time convention, and its MD5 matches staging and the committed file (apart from the final newline).
  - Before and after: 2 weddings, none published, 0 replies. Only the 8-argument `submit_shared_rsvp` exists; `rsvp_catering_summary` isn't callable by anon.
  - **Order slip:** the image went live about 2.5 minutes before the migration. `/dashboard` failed four times ("Unable to load wedding workspace", 19:00:36–19:01:03 UTC). No guest pages were affected, since none are published. There have been no errors since the migration, and `/api/health` and `/` answer 200. Next time, apply the migration before promoting the image (`docs/operations.md`).
- **Next step:** none for F068. The follow-ups are F069 (replies without JavaScript) and F070 (showcase page).
