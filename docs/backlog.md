...
- **Outstanding:** apply `supabase/migrations/` to the staging project (needs a Supabase access token or CLI login — not done in this session); then run the F038/operations.md staging journey, including real Stripe test Checkout and a Google sign-in test with a disposable or the owner's test-user account. Production Supabase, Render and Stripe live mode remain not started, each blocked on payment/owner authorisation as before.
- **Next:** apply the staging migrations, then run the staging journey check.
...
**Status:** Deferred for production, reopened for staging (owner decision, 26 September 2026). The code is built, tested and reviewed. `AUTH_GOOGLE_ENABLED=true` is now set on staging only; **Continue with Google** is live there but the Google consent screen is in Testing mode (owner's account only), not published — see the 26 September note below. Still not a paid-launch gate.
...
4. **A privacy notice that mentions Google sign-in** (wording requirement in `release-inputs.md` section 3). Still outstanding — blocks publishing the consent screen and marking this Done.
...
- **Next.** Either add specific disposable Google accounts as Testing-mode test users and run the item-5 checklist with those, or publish the consent screen (needs the F041 legal pages) first and then run it with genuinely disposable accounts. Production's own OAuth client is still not created.
