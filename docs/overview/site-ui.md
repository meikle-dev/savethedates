...
The workspace is split into sections, each with its own URL so it can be bookmarked and the back button works: **Overview** (`/dashboard`), **Basics** (names, date, location, note), **Design** (theme and photo framing), **Invitation** (F060), **Details**, **RSVP** (shared link, rotation, open/close settings), **Guests** (totals, paginated responses, corrections) and **Publish** (preview, purchase, guest link and its names part, publish/unpublish). Desktop (1024px and wider) shows a sticky left sidebar. Tablets (768-1023px) show all eight sections in one sticky row with no sideways scrolling. Phones (below 768px) show a compact sticky bar (64px or less) with the current section's icon and name and a **Sections** button (`aria-expanded`/`aria-controls`). The button opens a full-width list of all eight sections with the current one marked. The list closes when a section is chosen, on Escape, on a tap outside, on the button, and after back/forward. Escape and choosing a section return focus to the button. A tap outside leaves focus where the owner tapped (F029). The header always offers Preview and Sign out and shows names, date and status from 768px.

Overview shows only figures derived from saved data. Site status shows Published or Private draft, with a "Share your guest link" jump link and "online until" date when live. Published is green and Private draft orange, in the card and the header badge (F064). Offline keeps the neutral style. The countdown uses UTC calendar days. RSVPs show open/closed/off, total responses, and the attending/not-attending split. Latest responses shows up to five with a **View all guests** link. While the site is live, the guest link panel (below) follows the figures. Quick actions hold Preview. While a required step remains, a setup checklist leads the page and links to the right section: basics, optional photo, optional invitation, optional ceremony/reception details, RSVPs open, purchase, publish (F060 made Details optional). It disappears once all required steps are done. There is no guest-count target, since the product does not store an invited list.
...
### Setup and save clarity (F046, 25 September 2026)

- **Basics.** An empty wedding date says “Choose your wedding date.” A nonempty date outside the accepted range or impossible on the calendar says “Choose a valid date between 1900 and 2199.” A valid date before today shows a separate, nonblocking note beside the field: “This date has passed. Check it before you share your site.” Saving remains possible; this does not change purchase eligibility. After the first successful save, keep the existing confirmation visible and show a clear “Next: choose your style” link to Design beside or immediately below it. Do not redirect, show that onboarding link after a failed save, or repeat it when editing an existing wedding.
- **Photo framing.** Keep the selected page in the heading or nearby context. Label the page-specific controls “Reset to default crop” and “Revert to saved crop” so the former means the theme default and the latter means the last saved position. Use “Save framing for Save the Date” or “Save framing for Details” for the selected page.
- **Details.** Keep one “Show Details page” switch and the primary save action together after the fields. Beside them, state the **saved** visibility separately from the unsaved switch choice: “Saved Details are hidden from guests”, “Saved Details will appear when you publish”, or “Saved Details are visible on your live site”, as applicable. When the switch differs from the saved value, say “Save to apply this visibility change.” A save changes visibility only through that explicit switch choice; retain its stored value on edits. Keep the existing specific success message after save, and place errors near the affected fields. Ceremony and reception times remain free text; show examples such as “2:30 pm” and “From 6 pm” under the respective fields.
- **Phone save area.** Below 768px, use one sticky bottom area for the Details switch, saved status, save button and unsaved hint, with the single form submit action. Keep the secondary Preview link in the normal page flow. Give the form enough bottom space for its final field, notices and focused controls to scroll fully above the save area, including at 320px and with the on-screen keyboard. Respect the device safe area. If the keyboard leaves too little viewport height, let the save area return to document flow rather than obscure content. At wider widths, the same controls sit in the form flow without a sticky overlay.

### Optional guest pages and the Invitation section (F060, 25 September 2026)
...
- **Checklist.** Details stops being a required step. "Add ceremony or reception details" and "Set up your invitation" are optional steps, done when that page is switched on.
- **Invitation section** (`/dashboard/invitation`, after Design in the navigation). It's one bounded form, mirroring Details:
  - **Wording:** host line, invitation wording and afterwards line. Each shows a character count, and the wording field shows the standard phrase as its default.
  - **When and where:** ceremony time, venue and address, with the note "Also shown on your Details page".
  - **Save area:** the "Show Invitation page" switch, the saved-visibility line ("Saved Invitation is hidden from guests", "… will appear when you publish" or "… is visible on your live site"), the save button, and the unsaved hint.
  - **Preview saved Invitation** link.
  - The switch's help text says it's fine to leave it off when sending printed invitations.
...
- **RSVP readiness (F044).** One wording (`rsvpShareStatus`) covers draft, off, on, closed-by-date and offline (published but the purchase has expired) states, before and after publishing. The RSVP settings panel shows it as a badge and sentence and repeats it in the save notice. Before publishing, Publish shows it beside the future link; when guests could not reply (off or date passed) it is a warning that links to RSVP settings and says the site can still be published as an announcement only. Publishing never changes RSVP settings. Overview adds the sentence to the RSVP figure when RSVP is off or closed and there is no live panel, and shows **Offline** for expired sites.
