...
The authenticated `/dashboard/preview/rsvp` route renders the shared RSVP form using only the signed-in owner's saved names and candidate theme, even before publication or enabling RSVP. It sends no RSVP action and carries no guest secret. Save the Date and Details preview navigation retains the candidate theme when visiting RSVP. Visiting a published wedding's public RSVP path redirects only its verified owner to their live shared RSVP link (the same page guests use, so a submission there is saved); anonymous users and other owners see the unavailable state.
