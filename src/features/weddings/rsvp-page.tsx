...
import { submitRsvp, submitSharedRsvp } from "@/features/workspace/rsvp-actions";
import { WeddingFrame, WeddingHeader, WeddingFooter } from "./wedding-frame";
import { BotanicalArt } from "./wedding-art";

export function RsvpPage({ wedding, guest, slug, token, sharedSecret, previewHrefs }: { wedding: { first_name: string; second_name: string; theme: GuestRsvp["theme"]; details_enabled: boolean; rsvp_enabled: boolean }; guest: GuestRsvp | null; slug: string; token: string | null; sharedSecret?: string | null; previewHrefs?: ReturnType<typeof weddingJourneyHrefs> }) {
  const [state, action, pending] = useActionState<RsvpState, FormData>(sharedSecret ? submitSharedRsvp : submitRsvp, {});
  const respondingName = state.respondingName ?? guest?.responding_name ?? "";
  const attending = state.attending ?? guest?.attending;
  const unavailable = !previewHrefs && (!(token || sharedSecret) || !guest);
  const open = !!previewHrefs || !!guest?.is_open;
  const hrefs = previewHrefs ?? weddingJourneyHrefs(slug, token, sharedSecret);
  ...
  return <WeddingFrame theme={wedding.theme} className="details-shell rsvp-shell">
    ...
    <main id="main" className="rsvp-main">
      <div className="rsvp-intro">
        ...
        <p className="rsvp-invitation-context">{previewHrefs ? "Guests enter their names to RSVP with your shared link." : unavailable ? "Open the private RSVP link sent by the couple to respond." : sharedSecret ? "Please enter your name and answer below." : `This invitation is for ${guest?.invite_name}.`}</p>
      </div>
      <section className="rsvp-card" data-state={cardState} aria-label="Invitation response">
        ...
        </div> : sharedSecret && state.success ? <div className="rsvp-state" role="status"><h2 className="editorial text-2xl">Thank you</h2><p className="form-notice mt-5">{state.message}</p></div> : <form action={previewHrefs ? undefined : action} onSubmit={previewHrefs ? (event) => event.preventDefault() : undefined} noValidate>
          {!previewHrefs && <><input type="hidden" name="slug" value={slug} />{sharedSecret ? <input type="hidden" name="secret" value={sharedSecret} /> : <input type="hidden" name="token" value={token ?? ""} />}</>}
