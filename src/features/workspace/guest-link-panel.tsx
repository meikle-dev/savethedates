...
export function GuestLinkPanel({ url, message: suggested, availability, closesOn, justPublished = false }: GuestLinkShare & { justPublished?: boolean }) {
  ...
  return <section id="guest-link" className="ws-panel" aria-labelledby="guest-link-title">
    ...
    <p className="ws-panel-intro">This one link opens your Save the Date, and your Invitation, Details and RSVP pages while they’re on. Anyone who has it can view your site{availability === "open" ? " and reply" : ""}, so share it only with your guests. To stop it working, <Link href="/dashboard/rsvp" className="text-link">replace it in RSVP</Link>.</p>
