...
export function PublicationForm({ slug, published, locked, entitlement, checkout }: { slug: string | null; published: boolean; locked: boolean; entitlement: Entitlement; checkout?: string }) {
  ...
  return <>
    ...
    {published ? <>
      ...
      <label className="mt-6 flex items-start gap-3 text-sm leading-relaxed"><input name="visibility" type="checkbox" required checked={visibility} onChange={(event) => setVisibility(event.target.checked)} className="mt-1 size-5 shrink-0" /><span>I understand that anyone with the URL can view and copy our names, date, location, message and photo. Unpublishing cannot recall downloaded copies.</span></label>
      <button className="button button-primary mt-6" disabled={publishPending}>{publishPending ? "Publishing…" : "Publish site"}</button>
