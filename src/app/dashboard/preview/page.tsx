...
export default async function Preview({ searchParams }: { searchParams: Promise<{ theme?: string }> }) {
  ...
  const { data, error } = await client.from("weddings").select("first_name, second_name, wedding_date, location, message, photo_path, photo_framing, theme, published, details_enabled, rsvp_enabled, rsvp_closes_on, invitation_enabled").eq("owner_id", user.id).maybeSingle();
