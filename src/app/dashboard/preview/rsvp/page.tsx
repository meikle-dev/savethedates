...
export default async function RsvpPreview({ searchParams }: { searchParams: Promise<{ theme?: string }> }) {
  ...
  const { data, error } = await client.from("weddings").select("first_name, second_name, theme, details_enabled, rsvp_enabled, rsvp_closes_on, published, invitation_enabled").eq("owner_id", user.id).maybeSingle();
