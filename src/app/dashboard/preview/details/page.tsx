...
export default async function DetailsPreview({ searchParams }: { searchParams: Promise<{ theme?: string }> }) {
  ...
  const { data, error } = await client.from("weddings").select("first_name, second_name, theme, photo_path, details_enabled, ceremony_time, ceremony_venue, ceremony_address, ceremony_url, reception_time, reception_venue, reception_address, reception_url, travel, travel_url, accommodation, accommodation_url, dress_code, faqs").eq("owner_id", user.id).maybeSingle();
  ...
  return <>
    ...
    <WeddingDetailsPageView details={details} image={data.photo_path ? { src: "/dashboard/photo", alt: "" } : undefined} homeHref={`/dashboard/preview${query}`} detailsHref={`/dashboard/preview/details${query}`} previewEmpty />
  </>;
}
