...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ signout?: string }> }) {
  ...
  const { data, error } = await client.from("weddings").select("first_name, second_name, wedding_date, location, message").eq("owner_id", user.id).maybeSingle();
  ...
  return <div className="platform min-h-svh">
    ...
    <main className="mx-auto max-w-4xl px-6 pt-8 pb-20 md:pt-14">
      <div className="flex flex-wrap items-center gap-3"><p className="eyebrow">Your wedding workspace</p><span className="draft-badge">Private draft</span></div>
      ...
      <section aria-labelledby="draft-title" className="mt-10 border-t border-[var(--line)] pt-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-[var(--muted)]">Only you can access this draft. It isn’t shared with guests.</p>
        <DraftForm initial={draft} />
      </section>
      <aside className="mt-12 border-t border-[var(--line)] pt-6">
        <h2 className="text-sm font-semibold">What comes next?</h2>
        <p className="mt-2 max-w-xl text-sm leading-relaxed text-[var(--muted)]">Save your details now. Photo uploads and publishing your wedding website will be available in a later update.</p>
      </aside>
    </main>
  </div>;
}
