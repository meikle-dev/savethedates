...
export default async function ExampleLayout({ children, params }: { children: React.ReactNode; params: Promise<{ theme: string }> }) {
  ...
  return <><div className="platform example-banner"><a href="/#themes">← All themes</a><p><strong>{themes.find((item) => item.id === theme)?.name}</strong><span>Fictional wedding example</span></p><Link className="primary-button" href="/account/sign-up">Start your site</Link></div>{children}</>;
}
