...
export default async function WeddingPage({ params }: { params: Promise<{ weddingSlug: string }> }) {
  ...
  if (!wedding) notFound();
  return <SaveTheDate wedding={wedding} homeHref={`/${weddingSlug}`} detailsHref={row?.details_enabled ? `/${weddingSlug}/details` : undefined} rsvpHref={row?.rsvp_enabled ? `/${weddingSlug}/rsvp` : undefined} />;
}
