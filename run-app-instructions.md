...
The themes differ only in CSS, so behaviour tests use one theme. Every theme is covered by `tests/theme-design.spec.ts` (Save the Date and Details at 320–1440px, image failures), `tests/rsvp-preview.spec.ts` (the RSVP form at the project width and 320px), `tests/rsvp.spec.ts` (the closed RSVP notice), `tests/meal-choices-themes.spec.ts` (the RSVP meal and dietary questions on each theme's example page, at the project width and 320px) and `tests/themes.spec.ts` (long content with a failed photo). Tests keep Playwright's 30-second limit: split a test that needs longer instead of raising its limit.
